...
import { loadCookie, sortTimezones } from "../timezone/index";
import "./clock_list.css";

const SortableItem = SortableElement(({ value, update }) => {
  return (
    <li>
      <Clock timezoneName={value} update={update} />{" "}
    </li>
  );
});
...
class ClockList extends Component {
  ...
  render() {
    if (!this.props.timezones.length) {
      return (
        <div className="clock-list-loading">Add Clocks to see the time</div>
      );
    }
    return (
      <div>
...
export default connect(mapStateToProps, { loadCookie, sortTimezones })(
  ClockList
);
